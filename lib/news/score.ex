defmodule News.Score do
  ...
  def update_score(type, model) when is_binary(type) do
    result = Repo.all from v in Vote,
      where: v.votable_type == ^type and v.votable_id == ^model.id,
      select: {sum(v.vote), count(v.vote)}
  ...
  def update_user_score(user=%User{}) do
    ...
    story_score = Enum.reduce(stories, 0.0, fn(story, acc) -> acc + (story.score||0.0) end)
    comm_score = Enum.reduce(comments, 0.0, fn(comment, acc) -> acc + (comment.score||0.0) end)
